...
- Banned antd components: `Layout` `Grid(Row/Col)` `Flex` `Space` `Card` `Skeleton` `Masonry` `Popconfirm` `Watermark` `Typography` `List` `Listy` `QRCode` `Tour` `Statistic` `FloatButton` `Mentions` `Descriptions` `Affix` `Avatar` `Transfer` `Result`. If needed, use pure H5 or existing component combinations instead.
...
- Banned antd components: `Layout` `Grid(Row/Col)` `Flex` `Space` `Card` `Skeleton` `Masonry` `Popconfirm` `Watermark` `Typography` `List` `Listy` `QRCode` `Tour` `Statistic` `FloatButton` `Mentions` `Descriptions` `Affix` `Avatar` `Transfer` `Result` — use H5 or existing components when needed — see Import contract
