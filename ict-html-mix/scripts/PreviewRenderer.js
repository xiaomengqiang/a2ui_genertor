...
    const scriptUrls = meta.scripts.map(src => src.startsWith('./') ? `${distPath}/${src.slice(2)}` : src);
    ...
    if (this.options.data) {
      ...
      // 注意：本地化页拷贝清单不含 data.js，该回退仅项目根运行时适用——
      // 缺失必须显式报错；若静默吞掉，串行链会沿用上一节点残留的全局数据（张冠李戴）。
      if (this._isFileProtocol()) {
        ...
        if (!PreviewRenderer._defaultDataJs) {
          ...
          if (!dr.ok) {
            throw new Error(`Default data not found: ${distPath}/data.js (localized pages do not ship previewdist/data.js; pass dataPath or data instead)`);
          }
          PreviewRenderer._defaultDataJs = await dr.text();
        }
        delete window.__A2UI_DATA__; // 清残留，确保读到的必是本次默认数据
        try { (new Function(PreviewRenderer._defaultDataJs))(); } catch (e) {
          throw new Error(`Failed to execute default data.js (${distPath}/data.js): ${e && e.message}`);
        }
        if (typeof window.__A2UI_DATA__ === 'undefined' || !window.__A2UI_DATA__) {
          throw new Error(`data.js loaded but window.__A2UI_DATA__ missing/empty: ${distPath}/data.js`);
        }
      }
    }
