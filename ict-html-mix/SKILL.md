...
> 1. **结构登记**：页尾 `nodes` 数组项增删、路线 B 的新槽位节点插入、渲染器 `script` 标签与 `?v=` bump。
> 2. **路线 A 容器整备**（把布局控制权交给 data.js 的必要步骤，方法见 WORKFLOW.md Pitfall #7/#8/#11）：清空容器布局类、加 `style="display:block"` 覆盖、迁移/移除容器视觉类。
>
> 已渲染内容的微调走路线 C（data.js 层 patch，遵守 ict-coder 的最小变更纪律：只改用户所述，其余字节不动）。

> 技能文件夹：本文件 = 执行手册（操作自包含）；`WORKFLOW.md` = 参考手册（渲染器行为 /
> file:// 原理 / Pitfalls 排障（含布局陷阱 #7–#11）/ 存储规则，按需查阅）；`scripts/` = **渲染器唯一权威源
...
存储布局唯一：**本地化**——数据落 `<页目录>/a2ui-data/<slug>/data.js`（每节点独立文件夹，文件名统一为 `data.js`），页面引用前缀 `./`；运行时副本 `<页目录>/previewdist/`。无集中式数据布局；ict-coder 的中间 JSON 仅在生成过程中短暂存在（wrapper 写入且双校验均 PASS 后即删除），最终交付物只有 data.js。
...
> **⚠️ 生成前必须读取页面上下文**：在调用 ict-coder 生成新 data.js 之前，**必须先读取承载页中目标容器及周边兄弟节点、父节点**的 HTML 片段与生效 CSS，记录其实际尺寸（邻近卡片自然高度）、间距类（`gap-*`/`p-*`/`space-y-*`）、布局类（`grid`/`flex`/`grid-cols-*`）、视觉样式（padding/shadow/rounded/bg）、父子关系与 DOM 顺序。data.js 中的间距与视觉类必须取自这些实读值，不得凭空估算（视觉去重规则见 WORKFLOW.md Pitfall #11）。

**高度与间距速查表（唯一口径；原理、后果与推导见 WORKFLOW.md Pitfall #9/#10）**：
...
2. **贴合邻近语境造槽位**：
   ...
   - 独行成行 → 与同行卡片内容高度对齐（参考第 1 步速查表），靠 grid `align-items: stretch` 自动拉齐，不写死固定高度；
   - flex 父级 → 加 `flex-shrink-0` 防压缩塌缩；
   - 不与既有节点重叠、不压缩既有布局。
3. **内容高度预判**按第 1 步速查表与速查铁律执行（表格/列表按行数 × 48px/行估算，约束策略遵守铁律 2/3）。
...
5. **回退**：删除新 nodes 项 → 恢复旧 nodes 项（旧 data.js 未删，直接可用）→ 还原区域容器的布局类与被清空的原生内容（git）。
...
1. **反查 dataPath**：目标节点的 `dataPath` 登记在承载页页尾编排脚本的 `nodes` 数组里，用 **Grep** 工具检索（跨平台）：
   ...
   命中后 **Read** 该文件命中行号 ±3 行，即得 `container`/`dataPath` 登记项。类名选择器（如 `.card`）在全项目扫描时易误报——命中行必须位于页尾编排脚本的 `nodes` 数组内（同行含 `dataPath`）才算登记项，纯 DOM 出现的 class 命中不算。
...
8. **容器高度约束**：每次路线 A/B 挂载新内容到容器时，必须遵循第 1 步「速查表 + 速查铁律」——容器不写死固定高度、图表显式设高、表格分页或 `max-h` + `overflow-y-auto` 控高。任何因容器高度策略不当导致内容截断或撑开布局的问题，视为工作流执行缺陷。
9. **禁止直接编辑原生 HTML 内容**（见首要总则）：原生区域必须先经路线 A 转化为 A2UI 渲染容器，再走数据层修改。
10. **区域覆盖优先**：用户选中区域包含既有 A2UI 渲染子节点时，必须对**整个选中区域**做路线 A 整体替换——新 data.js 接管全区域布局与内容，并同步移除被覆盖的旧 nodes 项（见「路线 A 区域级替换」小节）；不得只针对内部子节点做局部修改来绕过区域级接管。
