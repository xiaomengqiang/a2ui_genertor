...
  if ($InputFile) { Show-Usage; exit 1 }
  ...
  # 优先用 locate-skill-root.ps1 解析技能根（兼容技能装在用户技能目录的场景）；失败/缺失兜底项目根 .opencode\skills
  $locateHelper = Join-Path $root 'locate-skill-root.ps1'
  $indexHtml = $null
  if (Test-Path -LiteralPath $locateHelper) {
    $sRoot = & powershell -NoProfile -File $locateHelper
    if ($sRoot) { $candidate = Join-Path $sRoot 'ict-coder\scripts\previewdist\index.prototype.html'; if (Test-Path -LiteralPath $candidate) { $indexHtml = $candidate } }
  }
  if (-not $indexHtml) { $indexHtml = Join-Path $projRoot '.opencode\skills\ict-coder\scripts\previewdist\index.prototype.html' }
