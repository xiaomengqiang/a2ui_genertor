# Select
> id: string | component: "Select" | props: object

## props (required: `value`, `options`)
- `value`: string | number | array | DataBinding — The current selected value(s).
- `options`: { `label`: string, `value`: string | number | boolean }[] | DataBinding — The dropdown options list.
- `size?`: "large" | "medium" | "small"
- `placeholder?`: string | DataBinding — Placeholder text.
- `showSearch?`: boolean
- `mode?`: "multiple"
